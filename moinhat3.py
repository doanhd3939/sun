...
# Lấy PORT từ biến môi trường để chạy trên Render
...
def index():
    # Bạn nên thay BYPASS_TEMPLATE thành nội dung HTML bạn muốn hiện ra
    return "<h2>Sun Bypass Service đang chạy. Hãy dùng Telegram bot!</h2>"
    ...
    if action == "bypass":
        ...
        async def delay_and_reply():
            ...
            def get_code():
                nonlocal result
                try:
                    resp = requests.post(f"http://localhost:{PORT}/bypass", json={"type": type, "user_id": user_id, "message": f"/ym {type}"})
                    data = resp.json()
...
if __name__ == "__main__":
    # Chạy Flask trên thread chính như Render yêu cầu web service phải chạy foreground
